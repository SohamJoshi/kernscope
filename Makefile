...
OUTPUT := .output
CLANG ?= clang
LIBBPF_SRC := $(HOME)/libbpf-bootstrap/libbpf/src
BPFTOOL_SRC := $(HOME)/libbpf-bootstrap/bpftool/src
...
# outdated
INCLUDES := -I$(OUTPUT) \
            -I$(HOME)/libbpf-bootstrap/libbpf/include/uapi \
            -I$(dir $(VMLINUX)) \
            -I$(LIBBLAZESYM_INC)
CFLAGS := -g -Wall
...
CLANG_BPF_SYS_INCLUDES += -I/usr/include/aarch64-linux-gnu
ifeq ($(V),1)
	Q =
	msg =
else
...
$(APPS): %: $(OUTPUT)/%.o $(LIBBPF_OBJ) | $(OUTPUT)
	$(call msg,BINARY,$@)
	$(Q)$(CC) $(CFLAGS) $^ $(ALL_LDFLAGS) -lelf -lz -lseccomp -o $@
